...
├── common/
│   ├── Compose.kt                    # @PreviewDark — аннотация для dark-mode превью
│   ├── Coroutines.kt                 # repeatOnStart / repeatOnCreate — extensions для Fragment
│   └── Kotlin.kt                     # String.empty, Date.toDMYFormat, Double.format
├── core/
│   ├── adapter/
│   │   └── SettingsAdapter.kt        # RadioButton-адаптер для диалогов выбора настроек
│   ├── customview/
│   │   └── CustomSwitchView.kt       # Кастомный переключатель (расходы/доходы)
│   ├── ui/
│   │   ├── components/               # Переиспользуемые Compose-компоненты
│   │   └── theme/                    # Compose Material3 тема
│   │       ├── Color.kt
│   │       ├── Shape.kt
│   │       ├── Type.kt
│   │       ├── Theme.kt              # DebtBookTheme + LocalDebtBookDarkTheme
│   │       ├── Spacing.kt            # Система отступов (space4…space48)
│   │       └── Elevation.kt          # Токены elevation (card, borderHairline) + LocalElevation
│   ├── MainActivity.kt               # Основная Activity с NavHostFragment
│   └── MainViewModel.kt              # Общий VM для MainActivity
└── di/
...
### ViewModels (14 штук)
Провайдятся автоматически через `@HiltViewModel`. Полный список: `DebtViewModel`, `MainViewModel`, `DebtDetailsViewModel`, `SettingsViewModel`, `CreateDebtViewModel`, `SynchronizationViewModel`, `FinanceViewModel`, `CreateFinanceViewModel`, `CreateFinanceCategoryViewModel`, `FinanceDetailsViewModel`, `GoalsViewModel`, `CreateGoalsViewModel`, `GoalDetailsViewModel`, `AuthorizationViewModel`.
...
Используется в `AuthorizationScreen` и всех Compose-экранах. `DebtBookTheme` — `@Composable` функция: `Color.kt` (цвета), `Shape.kt` (формы), `Type.kt` (типографика), `Theme.kt` (объединение + dark/light + провайдинг `LocalSpacing`, `LocalElevation`, `LocalDebtBookDarkTheme`).

**`LocalDebtBookDarkTheme`**: тот же флаг, что и аргумент `darkTheme` у `DebtBookTheme`. Для карточек/поверхностей использовать его вместо `isSystemInDarkTheme()`, чтобы при смене темы из настроек (будущее) не было рассинхрона с `MaterialTheme.colorScheme`.

### Spacing (`Spacing.kt`)

Система отступов, доступная через `MaterialTheme.spacing` из любого `@Composable`. Провайдится в `DebtBookTheme` вместе с `LocalElevation` и `LocalDebtBookDarkTheme` через `CompositionLocalProvider`.
...
Использование: `MaterialTheme.spacing.space16`

### Elevation (`Elevation.kt`)

Токены высоты тени и тонких обводок: `card` (4.dp), `borderHairline` (1.dp). Доступ: `MaterialTheme.elevation` (после `DebtBookTheme`).
...
Пакет кросс-фичевых утилит:

| Файл | Содержимое |
|------|-----------|
| `Compose.kt` | `@PreviewDark` — составная аннотация `@Preview(uiMode = UI_MODE_NIGHT_YES)` |
| `Coroutines.kt` | `Fragment.repeatOnStart { }` / `Fragment.repeatOnCreate { }` — collectAsState в нужном lifecycle-состоянии |
| `Kotlin.kt` | `String.empty`, `Date.toDMYFormat(pattern)`, `Double.format(pattern)` |

---

## SettingsAdapter (`core/adapter/`)

RecyclerView адаптер для отображения вариантов настроек в диалоге:
- Radio-style выбор (одно значение из списка)
- Используется в: настройках (валюты, тема), создании долга (валюта), финансах (валюта, интервал)
...
Launcher activity, хостит Jetpack Compose через `setContent { DebtBookTheme { AuthorizationScreen() } }`. Класс находится в `auth/` (не `auth/presentation/`).
...
Класс находится в `auth/` (не `auth/viewmodel/`). **Зависимости**: `GetPINCodeEnabled`, `SetPINCodeEnabled`, `GetPINCode`, `SetPINCode`, `GetIsFingerprintAuthEnabled`.
...
|----|---------|---------|
| `mainFragment` | `DebtFragment` | Список людей/долгов |
| `newDebtFragment` | `CreateDebtFragment` | Создание долга |
