...
Используется в `AuthorizationScreen` и всех Compose-экранах. `DebtBookTheme` — `@Composable` функция: `Color.kt` (цвета), `Shape.kt` (формы), `Type.kt` (типографика), `Theme.kt` (объединение + dark/light + провайдинг `LocalSpacing`).

### Spacing (`Spacing.kt`)

Система отступов, доступная через `MaterialTheme.spacing` из любого `@Composable`. Провайдится в `DebtBookTheme` через `CompositionLocalProvider(LocalSpacing provides Spacing())`.
