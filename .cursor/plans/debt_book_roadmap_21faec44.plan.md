...
- AGP `7.2.1`, Gradle `7.3.3`, Kotlin `1.9.0` — устарели на 2+ года
- Room `2.4.2`, Lifecycle `2.5.0` — устарели
- Compose BOM `2024.02.01` — только для экрана авторизации
- Koin `3.5.3` для DI
- RxJava 3 в ViewModels
...
- `[App.kt](app/src/main/java/com/breckneck/debtbook/app/App.kt)`: класс называется `sadfsafApp` вместо `App` (не компилируется без правок)
- Compose тема импортируется из `com.breakneck.pokedex.ui.theme` (copy-paste из другого проекта)
- Пакет domain/data имеет опечатку: `deptbook` вместо `debtbook`
- `targetSdk` в модуле `data` = `32`, а в `app` = `34`
...
### 1.1 Обновление Gradle / AGP / Kotlin

- Gradle Wrapper: `7.3.3` → `8.9+`
- Android Gradle Plugin: `7.2.1` → `8.8+`
- Kotlin: `1.9.0` → `2.1+`
- Java compatibility: `1.8` → `17`
- Переход с `KAPT` на `KSP` (для Room и Koin)
...
Фаза 2.3 (Design System) → Фаза 3.1 (UX) → Фаза 3.2 (фичи) → Фаза 3.3 (монетизация)
```
